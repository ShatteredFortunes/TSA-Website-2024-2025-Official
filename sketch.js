    ...
    for (var i = 0; i < items.length; i++) {
        ...
        (function(index, item) {
            removeButton.onclick = function () {
                ...
                if (current == "Samosas" || current == "Apfelkuchen" || current == "Scallion Pancakes") {
                    var temp = localStorage.getItem("totalCost");
...
//Appetizer Functionality
{
    ...
    var ScallionToAdd = 1;
    function decrementScallion() {
    if (ScallionToAdd > 0) {
        ScallionToAdd--;
        document.getElementById("quantityScallion").value = ScallionToAdd;
        console.log(ScallionToAdd);
    }
}
function incrementScallion() {
    if (ScallionToAdd >= 0) {
        ScallionToAdd++;
        document.getElementById("quantityScallion").value = ScallionToAdd;
        console.log(ScallionToAdd);
...
function openScallion() {
    document.getElementById("ScallionDisplay").style.display = "block";
}
function Scallion() {
    for (var k = 0; k < ScallionToAdd; k++) {
   ...
   currentList.push("Scallion Pancakes");
   localStorage.setItem("totalList", JSON.stringify(currentList));
   console.log(`Updated array: ${JSON.stringify(currentList)}`);
   document.getElementById("ScallionDisplay").style.display = "none";
   reload();
    }
    ScallionToAdd = 1;
   document.getElementById("quantityScallion").value = ScallionToAdd;
}
function closeScallion() {
    document.getElementById("ScallionDisplay").style.display = "none";
}
    var falafelToAdd = 1;
...
//This may cause an error for some reason, despite the fact that it DOES change the style display and it DOES read it.
...
function enterQuiz() {
    if (question1 == "Asian") {
        if (question2 == "Appetizer") {
            document.getElementById("youShouldGet").innerHTML = "You should get Scallion Pancakes!";
